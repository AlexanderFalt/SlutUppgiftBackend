...
export default function ManagerApplication() {
    ...
    const fetchUsers = useCallback(async() => {
        try { 
            const response = await api.get(`${API}/api/admin/role-raise`)
    ...
    const acceptApplication = async(id: string) => {
        try {
            await api.put(`${API}/api/admin/role-raise/${id}`, {})
            fetchUsers();
        } catch(e) {
            console.log(e)
        }
                            ...
                            <Grid key={index} sx={{width: "100%", marginBottom: "2%"}}>
                                <Paper elevation={6} sx={{width: "100%", height: "100%", borderRadius: 6, padding: "1.5%", border: "1px solid silver"}}>
                                    ...
                                    <CardContent sx={{display: "flex"}}>
                                        <Box sx={{width: "50%"}}>
                                            <Typography variant="body1" sx={{color: "#282828", fontWeight: "400", fontSize: "clamp(0.95rem, 2.5vw, 1rem)"}}> Address: <Box component={'span'} sx={{fontWeight: "500"}}>{user.role}</Box></Typography>
                                        </Box>
                                        <Box sx={{width: "50%"}}>
                                    ...
                                    <Button variant="outlined" color="primary" onClick={() => acceptApplication(user.id)} sx={{width:"100%", height: "5vh", borderRadius: 6, margin: 0, marginRight: "1.5%", marginTop: "2%"}}>
                                        Approve Application
                                    </Button>
                                </Paper>
                            </Grid>
